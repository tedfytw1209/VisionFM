# Finetuning VisionFM on a public fundus-photo benchmark dataset.
#
# Data loading mirrors MIRAGE's run_cls_tuning_fundus.py (already verified
# working against these datasets on the Linux server): pre-split
# train/val/test/Class_x/ image folders, loaded with torchvision's
# ImageFolder and num_classes auto-inferred from the train split's
# subfolders. This is deliberately NOT the RETFoundDataset class in
# finetune_visionfm_for_multiclass_classification_UF.py, which hardcodes an
# exact class-folder-name list per dataset (unverified for some datasets and
# fragile if a folder name doesn't match).
...
def build_dataset(subset, args):
    ...
    dataset = datasets.ImageFolder(root, transform=build_transform(subset, args))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser('Finetuning VisionFM on a public fundus-photo benchmark dataset')
    ...
    parser.add_argument('--pretrained_weights', default='', type=str, required=True,
        help='Path to pretrained VisionFM weights.')
