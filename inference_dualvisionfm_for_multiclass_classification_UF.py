...
def eval_linear(args):
    ...
    oct_dataset_val = build_dataset(args=args, is_train='test', img_dir=args.img_dir, modality='OCT')
    oct_val_loader = torch.utils.data.DataLoader(
        oct_dataset_val,
    ...
    print(f"Data loaded with {len(oct_dataset_val)} test imgs.")

    fundus_dataset_val = build_dataset(args=args, is_train='test', img_dir=args.img_dir, modality='Fundus')
    ...
    print(f"Data loaded with {len(fundus_dataset_val)} test imgs.")
    
    #Assert target in same order
    print('Asserting the targets are in the same order for both modalities...')
    print('OCT targets: ', oct_dataset_val.targets[:10])
    print('Fundus targets: ', fundus_dataset_val.targets[:10])

    # ============ building network ... ============
    # OCT
    oct_model = models.__dict__[args.arch](
        img_size=[args.input_size],
        patch_size=args.patch_size,
        num_classes=0,
        use_mean_pooling=args.avgpool_patchtokens == 1)
    embed_dim = oct_model.embed_dim
    oct_model.cuda()
    print(f"Model {args.arch} {args.patch_size}x{args.patch_size} built.")
    # load visionfm pretrained weights
    utils.load_pretrained_weights(oct_model, args.oct_pretrained_weights, args.checkpoint_key, args.arch, args.patch_size)

    oct_linear_classifier = ClsHead(embed_dim=embed_dim * 4, num_classes=args.num_labels, layers=3)
    oct_linear_classifier = oct_linear_classifier.cuda()
    oct_linear_classifier = nn.parallel.DistributedDataParallel(oct_linear_classifier, device_ids=[args.gpu])

    # load the weights
    state_dict = torch.load(args.oct_pretrained_weights, map_location="cpu")
    ...
    if state_dict is None:
        ...
        msg = oct_linear_classifier.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(args.oct_pretrained_weights, msg))

    oct_model.eval()
    oct_linear_classifier.eval()
    # Fundus
    fundus_model = models.__dict__[args.arch](
        img_size=[args.input_size],
        patch_size=args.patch_size,
        num_classes=0,
        use_mean_pooling=args.avgpool_patchtokens == 1)
    embed_dim = fundus_model.embed_dim
    fundus_model.cuda()
    print(f"Model {args.arch} {args.patch_size}x{args.patch_size} built.")
    # load visionfm pretrained weights
    utils.load_pretrained_weights(fundus_model, args.fundus_pretrained_weights, args.checkpoint_key, args.arch, args.patch_size)

    fundus_linear_classifier = ClsHead(embed_dim=embed_dim * 4, num_classes=args.num_labels, layers=3)
    fundus_linear_classifier = fundus_linear_classifier.cuda()
    fundus_linear_classifier = nn.parallel.DistributedDataParallel(fundus_linear_classifier, device_ids=[args.gpu])

    # load the weights
    state_dict = torch.load(args.fundus_pretrained_weights, map_location="cpu")
    state_dict = state_dict.get('classifier_state_dict',None)
    if state_dict is None:
        print("Cannot find the weights for classifier (decoder). Please refer to our fine-tuning instruction!!")
        print("The classifier would utlize the random weights!!")
    else:
        msg = fundus_linear_classifier.load_state_dict(state_dict, strict=False)
        print('Pretrained weights found at {} and loaded with msg: {}'.format(args.fundus_pretrained_weights, msg))

    fundus_model.eval()
    fundus_linear_classifier.eval()
    
    # evaluate
    test_stats, output, target, output_labels = validate_network_dual(oct_val_loader, fundus_val_loader, oct_model, fundus_model, oct_linear_classifier, fundus_linear_classifier, args.n_last_blocks, args.avgpool_patchtokens)
...
@torch.no_grad()
def validate_network_dual(val_loader1, val_loader2, model1, model2, linear_classifier1, linear_classifier2, n, avgpool):
    model1.eval()
    model2.eval()
    linear_classifier1.eval()
    linear_classifier2.eval()
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Test:'
    targets, preds = [], []
    output_labels = []
    total = len(val_loader1)
    it_1 = iter(val_loader1)
    it_2 = iter(val_loader2)
    for _ in metric_logger.log_every(range(total), 20, header):
        inp1, target1 = next(it_1)
        inp2, target2 = next(it_2)
        # move to gpu
        ...
        # forward model 1
        with torch.no_grad():
            intermediate_output = model1.get_intermediate_layers(inp1, n)
            if avgpool == 0:
        ...
        model1_output = linear_classifier1(output)
        
        # forward model 2
        with torch.no_grad():
            intermediate_output = model2.get_intermediate_layers(inp2, n)
            if avgpool == 0:
                output = [x[:, 0] for x in intermediate_output]
            elif avgpool == 1:
                output = [torch.mean(intermediate_output[-1][:, 1:], dim=1)]
                ...
                assert False, "Unkown avgpool type {}".format(avgpool)
            output = torch.cat(output, dim=-1)
        model2_output = linear_classifier2(output)
        ...
        if num_class > 1:  # multi-class case
            ...
            loss = nn.BCEWithLogitsLoss()(output.squeeze(dim=1), target1.float())
        ...
        if num_class > 1:  # multi-classes
            ...
            targets.append(np.expand_dims(target1.detach().cpu().numpy(), axis=1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--oct_pretrained_weights', default='', type=str, help="""Path to pretrained 
        weights""")
    parser.add_argument('--fundus_pretrained_weights', default='', type=str, help="""Path to pretrained 
        weights""")
